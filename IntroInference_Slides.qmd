...
## When is a Sampling Distribution Normal? {.smaller}
...
2. Since this is the case, we'll check the *success-failure condition*...<br/>$\begin{array}{lcl} n\cdot p & = & 150\cdot\left(0.97\right) = 145.5 \geq 10~\checkmark\\
...
n\cdot \left(1 - p\right) & = & 500\cdot\left(0.03\right) = 15 \not\geq 10~\checkmark \end{array}$

3. The *success-failure condition* is satisfied, so the CLT says that the samplind distribution for the proportion will be $\displaystyle{N\left(p,~S_E = \sqrt{\frac{p\left(1 - p\right)}{n}}\right)}$
