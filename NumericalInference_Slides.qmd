...
iv) $S_E = s/\sqrt{n} \approx 27.39,~~~\text{df} = n - 1$
v) $t = \frac{450 - 400}{27.39} \approx 1.83$
vi) $p$-value $\approx$ `2*(1 - pt(1.83, df = 29))` $\approx$ `r 2*(1 - pt(1.83, df = 29))`

:::

vii) $p$-value $\geq \alpha$, fail to reject $H_0$
viii) The sample data is compatible with a reality in which the average annual repair costs are no more than \$400.
