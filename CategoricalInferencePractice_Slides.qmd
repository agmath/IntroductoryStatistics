...
5. Use [the Standard Error Decision Tree](https://agmath.github.io/SiteFiles/StdErrorDecisionTree.pdf){target="_blank"} to find how to compute the *standard error* ($S_E$)
...
5. Compute the *test statistic*, where $\displaystyle{\texttt{test statistic} = \frac{\left(\texttt{point estimate}\right) - \left(\texttt{null value}\right)}{S_E}}$
    ...
    + The *standard error* ($S_E$) formula is found on [the Standard Error Decision Tree](https://agmath.github.io/SiteFiles/StdErrorDecisionTree.pdf){target="_blank"}
