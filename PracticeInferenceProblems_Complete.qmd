...
x_vals_shade <- seq(qchisq(0.95, df = 4 - 1), 10, length.out = 100)
...
ggplot() + 
  ...
  geom_ribbon(aes(x = x_vals_shade, 
                  ymin = 0, 
                  ymax = y_vals_shade), fill = "skyblue") +
  scale_x_continuous(breaks = 0, labels = expression(mu == 8)) + 
  scale_y_continuous(breaks = NULL, labels = NULL) +
  labs(title = TeX(r'(Shaded Region is Samples Favorable to $H_a$)'),
       x = "",
       y = "") +
...
ggplot() + 
  ...
  geom_ribbon(aes(x = x_vals_shade2, 
                  ymin = 0, 
                  ymax = y_vals_shade2), fill = "skyblue") +
  scale_x_continuous(breaks = 0, labels = expression(mu[1.00] - mu[0.99] == 0)) + 
  scale_y_continuous(breaks = NULL, labels = NULL) +
  labs(title = TeX(r'(Shaded Region is Samples Favorable to $H_a$)'),
       x = "",
       y = "") +
...
p1 <- ggplot() + 
  geom_line(aes(x = x_vals, y = y_vals)) + 
  scale_x_continuous(breaks = c(0, test_stat), labels = c(expression(mu[1.00] - mu[0.99] == 0), 2.82)) + 
  scale_y_continuous(breaks = NULL, labels = NULL) +
  labs(title = "Our Test Statistic",
       x = "",
       y = "") + 
