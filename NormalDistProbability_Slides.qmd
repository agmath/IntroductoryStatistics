...
Luckily, we don't evaluate or work with this by hand either!
...
ggplot() + 
  ...
  geom_point(aes(x = x1, y = dnorm(x1, mean = mu3, sd = 1)),
             color = "orange") + 
  labs(
    title = "Normal Distributions with Different Means",
    x = "x",
    y = ""
  )
```
...
The picture to the right shows $\mathbb{P}\left[Z \geq z^*\right]$
...
ggplot() + 
  ...
  geom_line(aes(x = x, y = dnorm(x)),
            lwd = 1.5) + 
  scale_x_continuous(breaks = c(-2, 0, 1, 2), labels = c(-2, 0, "z*", 2)) +
  labs(
    x = "Z",
    y = ""
  )
...
ggplot() + 
  ...
  geom_line(aes(x = x, y = dnorm(x)),
            lwd = 1.5) + 
  scale_x_continuous(breaks = c(-2, -1, 0, 2), labels = c(-2, "-z*", 0, 2)) +
  labs(
    title = "P[Z < -z*]",
    x = "Z",
    y = ""
...
The probability that $X$ takes on a value *less than or equal to* (or just *less than*) $k$ is $\mathbb{P}\left[X \leq k\right] \approx$ `pbinom(k, mean = mu, sd = sigma)`
...
Since we have our boundary values, and we are looking for a probability, let's use `pnorm()`.
